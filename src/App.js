...
function generateShellReport({title, icon, target, attacks, results}) {
  ...
  doc.text(`Generated by VulnusLab — automated ${_safe(title).toLowerCase()} testing powered by Kali Linux.`, W/2, y+16, {align:"center"});
...
function generateOsintPdf(results, target) {
  ...
  txt("Generated by VulnusLab — automated OSINT gathering powered by Kali Linux.",pageW/2,y+14,6.5,GRAY,false,"center");
...
function generateReconReport({target, allResults, date}) {
  ...
  txt("— END OF REPORT —",pageW/2,bY+8,10,BLUE,true,"center");
  txt("Generated by VulnusLab — automated reconnaissance powered by Kali Linux.",pageW/2,bY+14,6.5,GRAY,false,"center");
  txt("All findings require manual verification. This document is CONFIDENTIAL — restricted to authorized personnel only.",pageW/2,bY+18,6,GRAY,false,"center");
  txt("vulnuslab.com  ·  support@vulnuslab.com",pageW/2,bY+25,7,BLUE,true,"center");
...
const EXPLOIT_TARGETS = [
  ...
  // ── Metasploitable 2 (one container, many vulnerable services) ──
  { label:"vsftpd 2.3.4",   os:"🐧", ip:"lab_metasploitable", port:21,   service:"vsftpd 2.3.4",   cve:"CVE-2011-2523", msf:"exploit/unix/ftp/vsftpd_234_backdoor",       payload:"cmd/unix/interact",           fmt:"raw", search:"vsftpd 2.3.4 backdoor", desc:"🟢 LIVE Docker — Smile-username backdoor → instant root shell" },
  { label:"Samba 3.x",      os:"🐧", ip:"lab_metasploitable", port:139,  service:"Samba 3.x",      cve:"CVE-2007-2447", msf:"exploit/multi/samba/usermap_script",         payload:"cmd/unix/reverse",            fmt:"raw", search:"samba usermap_script", desc:"🟢 LIVE Docker — Samba usermap shell command injection (port 139)" },
  { label:"UnrealIRCd",     os:"🐧", ip:"lab_metasploitable", port:6667, service:"UnrealIRCd 3.2.8.1", cve:"CVE-2010-2075", msf:"exploit/unix/irc/unreal_ircd_3281_backdoor", payload:"cmd/unix/reverse",         fmt:"raw", search:"UnrealIRCd backdoor",   desc:"🟢 LIVE Docker — IRC backdoor: AB; prefix triggers command exec" },
  { label:"distccd",        os:"🐧", ip:"lab_metasploitable", port:3632, service:"distccd v1",     cve:"CVE-2004-2687", msf:"exploit/unix/misc/distcc_exec",              payload:"cmd/unix/reverse",            fmt:"raw", search:"distcc exec",          desc:"🟢 LIVE Docker — Distributed compiler RCE — instant shell" },
];
...
function generateExploitReport({target, port, service, cve, msfModule, msfPayload, lhost, lport, searchResults, vulnResults, msfResult, payloadResult, shellLog, date}) {
  const _doGen = () => {
    ...
    // ── Cover ────────────────────────────────────────────────────
    fillR(0,0,pageW,56,DARK);
    fillR(0,56,pageW,2,BLUE);
    // VulnusLab shield logo + wordmark (left side of banner)
    {
      const sx = 22, sy = 16, w = 10, h = 12;
      doc.setFillColor(15,23,42);
      ...
      doc.setTextColor(241,245,249);
      doc.text("VULNUS", sx - 1, sy + 9, {align:"right"});
      doc.setTextColor(...BLUE);
      doc.text("LAB", sx, sy + 9, {align:"left"});
    }
    doc.setFont("helvetica","bold");doc.setFontSize(20);doc.setTextColor(...BLUE);
    doc.text("EXPLOITATION REPORT",pageW/2,24,{align:"center"});
    doc.setFontSize(11);doc.setTextColor(...LIGHT);
    doc.text("Penetration Testing — Active Exploitation Phase",pageW/2,36,{align:"center"});
    doc.setFontSize(9);doc.setTextColor(...GRAY);
    doc.text(date,pageW/2,44,{align:"center"});
    y=68;
    ...
    // ── End of report contact block + footer ─────────────────────
    chk(34);
    const eY = y + 4;
    fillR(margin, eY, contentW, 30, [241,245,249]);
    fillR(margin, eY, 3, 30, BLUE);
    txt("— END OF REPORT —", pageW/2, eY+8, 10, BLUE, true, "center");
    txt("Generated by VulnusLab — automated exploitation testing powered by Kali Linux.", pageW/2, eY+14, 6.5, GRAY, false, "center");
    ...
    for(let p=1;p<=totalPages;p++){
      ...
      txt("VulnusLab | CONFIDENTIAL  ·  vulnuslab.com",margin,291,7,GRAY);
      txt(`Page ${p} of ${totalPages}`,pageW/2,291,7,GRAY,false,"center");
      txt("support@vulnuslab.com",pageW-margin,291,7,BLUE,false,"right");
    }
  ...
  const autoExploit = async () => {
    ...
    try {
      ...
      if(moduleEmpty || (isWebTarget && isSmbModule)){
        log(`ℹ️ Phase 4: Skipped — ${moduleEmpty ? "no MSF module configured" : "SMB/Windows module not compatible with web port "+targetPort}`);
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              {searchResult && (
                <div style={S.card}>
                        ...
                        <tbody>
                          {searchResult.exploits.slice(0,15).map((e,i)=>(
                            <tr key={i} style={{background:i%2===0?"#0f172a":"#111827"}}>
                              <td style={{padding:"3px 6px",color:"#64748b",fontFamily:"monospace"}}>{e.edb_id||"—"}</td>
                  ...
                  ) : <div style={{fontSize:11,color:"#64748b"}}>No exploits found — try a different search term</div>}
                </div>
              )}
              ...
              {vulnResult && (
                <div style={S.card}>
                  ...
                  ) : <div style={{fontSize:11,color:"#64748b"}}>No CVEs flagged — check raw output or try with all ports</div>}
                </div>
              )}
              ...
              {msfResult && (
                <div style={{...S.card,border:`1px solid ${msfResult.session_opened?"#16a34a":"#7f1d1d"}`}}>
                  ...
                  <div style={{fontSize:10,color:"#64748b",marginBottom:6}}>Module: {msfResult.module} · Payload: {msfResult.payload}</div>
          ...
          {(shellLid||shellStatus!=="idle") && (
            <div style={{marginTop:10,background:"#020617",border:`1px solid ${shellStatus==="connected"?"#22c55e":shellStatus==="waiting"?"#f59e0b":"#334155"}`,borderRadius:8,overflow:"hidden"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",background:"#0a0f1e",borderBottom:"1px solid #1e293b"}}>
                ...
                <button onClick={stopShell} style={{marginLeft:"auto",background:"none",border:"none",color:"#475569",fontSize:11,cursor:"pointer"}}>✕ Close</button>
              </div>
