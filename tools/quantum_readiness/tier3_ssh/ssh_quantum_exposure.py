...
packet straight off the socket and parses the advertised name-lists. No login
is attempted. Advisory/LOW - a future risk, never graded High. Any parse error
degrades to no finding (conservative).
"""
import asyncio
import socket
...
def _read_kexinit(host, port=_PORT, timeout=6):
    """Return (kex_algorithms[list], host_key_algorithms[list]) or (None, None)."""
    try:
        s = socket.create_connection((host, port), timeout=timeout)
        s.settimeout(timeout)
        # Server identification string(s) end with the first line containing "SSH-".
        banner = b""
        ...
        while len(data) < 4096:
            ...
            if not chunk:
                break
            data += chunk
            if len(data) >= 6 and data[5] == 20:   # SSH_MSG_KEXINIT seen
