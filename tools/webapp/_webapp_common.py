...
What this fixes (after the Forge audit):
  Every webapp scanner had its own bespoke response handling. Customer PDFs
...
Two helpers:
  ...
  vuln_response(tool, target, findings, tested, what_checked, ...)
      Drop-in replacement for standard_response() in webapp scanners.
...
def precheck_target(base: str, req, *, active_probes: bool = True) -> Optional[str]:
    ...
    Used by every webapp scanner BEFORE expensive probing — if the target's
...
def vuln_response(
    *,
    ...
    Use this in place of standard_response() in webapp scanners. When the
