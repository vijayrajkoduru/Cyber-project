...
def rule_handshake_captured(s):
    if not s.get("handshake_captured"):
        return None
    return {"name": "WPA/WPA2 4-way handshake CAPTURED — PSK is offline-crackable",
            "severity": "HIGH", "cvss": "7.5",
            "cwe": "CWE-326", "owasp": "A02:2021",
            "evidence": ("airodump captured " + str(s.get("eapol_packets", 0)) +
                         " EAPOL frames for target " + str(s.get("target_label", "")) +
                         " — sufficient for hashcat mode 22000 brute force"),
