"""email_to_phone_pivot — VL-FORGE OSINT (key-gated graceful)."""
import asyncio, os
...
async def gather(ctx):
    ...
    candidates=["EMAIL_TO_PHONE_PIVOT_API_KEY","EMAIL_TO_PHONE_PIVOT_KEY","INTELX_API_KEY","LEAKCHECK_API_KEY","DEHASHED_API_KEY"]
    key=next((os.environ.get(c) for c in candidates if os.environ.get(c)),None)
    ctx.state["api_key_configured"]=bool(key)
    if not key:
        ctx.state["skipped_reason"] = "Requires EmailRep / PhoneRep API key — set env EMAILREP_API_KEY to enable. Free tiers available at vendor's site."
        return
    ctx.state["host"]=ctx.host
    if key: ctx.source("email_to_phone_pivot-keyed")
def _r_unkeyed(s):
...
def _r_keyed(s):
    ...
    return {"name":"email_to_phone_pivot configured","severity":"INFO",
...
async def f(req:ScanRequest,_=Depends(verify_scan_quota)):
    return await run_scanner(host=recon_host(req.target),tool="email_to_phone_pivot",
