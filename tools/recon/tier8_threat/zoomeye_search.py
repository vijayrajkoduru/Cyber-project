"""zoomeye_search — VL-FORGE Threat Intel."""
...
async def gather(ctx):
    candidates=["ZOOMEYE_SEARCH_API_KEY","ZOOMEYE_SEARCH_TOKEN","CENSYS_API_TOKEN","FOFA_KEY","QUAKE_KEY","ZOOMEYE_KEY","MISP_API_KEY"]
    key=next((os.environ.get(c) for c in candidates if os.environ.get(c)),None)
    ctx.state["api_key_configured"]=bool(key)
    if not key:
        ctx.state["skipped_reason"] = "Requires ZoomEye API key — set env ZOOMEYE_API_KEY to enable. Free tiers available at vendor's site."
        return
    if not key: return
    ctx.source("zoomeye_search-keyed")
...
def _r_keyed(s):
    ...
    return {"name":"zoomeye_search ready (intel query on demand)","severity":"INFO",
...
async def f(req:ScanRequest,_=Depends(verify_scan_quota)):
    return await run_scanner(host=recon_host(req.target),tool="zoomeye_search",
