"""shodan_keyed — VL-FORGE Recon (real, zero-FP)."""
...
async def gather(ctx: ScanContext):
    ...
    if not key:
        ...
        ctx.source("SHODAN_API_KEY not set"); return
    from tools.recon._threat_helpers import api_get
    url = f'https://api.shodan.io/shodan/host/search?key=' + key + '&query=hostname:' + h
    headers = {}
    ...
    ctx.state["threat_hit"] = bool((data or {}).get("matches"))
    ctx.source("shodan_keyed API probe")

RULES = [
    lambda s: {"name":"shodan_keyed: target flagged in threat intelligence","severity":"MEDIUM",
...
async def recon_shodan_keyed(req: ScanRequest, _=Depends(verify_scan_quota)):
    host = recon_host(req.target)
    return await run_scanner(host=host, tool="shodan_keyed",
