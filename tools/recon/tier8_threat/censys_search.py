"""censys_search — VL-FORGE Threat Intel."""
...
async def gather(ctx):
    candidates=["CENSYS_SEARCH_API_KEY","CENSYS_SEARCH_TOKEN","CENSYS_API_TOKEN","FOFA_KEY","QUAKE_KEY","ZOOMEYE_KEY","MISP_API_KEY"]
    key=next((os.environ.get(c) for c in candidates if os.environ.get(c)),None)
    ctx.state["api_key_configured"]=bool(key)
    if not key:
        ctx.state["skipped_reason"] = "Requires Censys.io API key — set env CENSYS_API_KEY to enable. Free tiers available at vendor's site."
        return
    if not key: return
    ctx.source("censys_search-keyed")
...
def _r_keyed(s):
    ...
    return {"name":"censys_search ready (intel query on demand)","severity":"INFO",
...
async def f(req:ScanRequest,_=Depends(verify_scan_quota)):
    return await run_scanner(host=recon_host(req.target),tool="censys_search",
