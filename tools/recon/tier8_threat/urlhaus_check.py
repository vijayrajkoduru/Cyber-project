"""urlhaus_check — VL-FORGE Threat Intel."""
...
async def gather(ctx):
    candidates=["URLHAUS_CHECK_API_KEY","URLHAUS_CHECK_TOKEN","CENSYS_API_TOKEN","FOFA_KEY","QUAKE_KEY","ZOOMEYE_KEY","MISP_API_KEY"]
    key=next((os.environ.get(c) for c in candidates if os.environ.get(c)),None)
    ctx.state["api_key_configured"]=bool(key)
    if not key: return
    ctx.source("urlhaus_check-keyed")
    ctx.state["data_available"]=True
def _r_unkeyed(s):
    if s.get("api_key_configured"): return None
    return {"name":"urlhaus_check requires API key","severity":"INFO",
        "evidence":"Set appropriate env var"}
def _r_keyed(s):
    if not s.get("api_key_configured"): return None
    return {"name":"urlhaus_check ready (intel query on demand)","severity":"INFO",
